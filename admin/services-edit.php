...
// Keep a copy of the original data for diffing
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $long_description  = $_POST['long_description']; // TinyMCE HTML
    ...
    if (!$error) {
        $stmt = $conn->prepare("
            UPDATE services 
            SET name=?, slug=?, category=?, price=?, short_description=?, long_description=?, image=?, is_active=?
            WHERE id=?
        ");
        $stmt->bind_param(
            "sssdsssii",
        ...
        if ($stmt->execute()) {
            ...
            if ($originalService['category'] !== $category) {
                $changes[] = "category: '{$originalService['category']}' → '{$category}'";
            }
            ...
            if ((int)$originalService['is_active'] !== (int)$is_active) {
                $oldStatus = $originalService['is_active'] ? 'active' : 'inactive';
            ...
            if (!empty($changes)) {
                ...
                $description = "Service {$originalService['name']} (ID {$id}) updated: " . implode('; ', $changes);
...
<head>
    ...
    <!-- Use your TinyMCE API key below -->
    ...
    <main class="main-content">
        <header class="admin-header">
            <div class="admin-header-left">
                <h1>Edit Service</h1>
                <p class="header-subtitle">Modify business service details and show/hide options.</p>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($service['image'])): ?>
                            <div style="margin-top: 8px;">
                                <img src="../<?= htmlspecialchars($service['image']) ?>" alt="Current Image" style="max-width:180px; border-radius:5px;">
